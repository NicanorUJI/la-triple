...
public class PacoManMovement : MonoBehaviour
{
    ...
    public string doorTag = "Door"; // opcional, si quieres puertas
    ...
    // Para control de inmortalidad tras recibir daño
    ...
    private void FixedUpdate()
    {
        ...
        if (nextDirection != Vector2.zero)
        {
            ...
            {
                bool isOpposite = Vector2.Dot(moveDirection.normalized, nextDirection.normalized) < -0.9f;
                bool isPerpendicular = Mathf.Abs(Vector2.Dot(moveDirection.normalized, nextDirection.normalized)) < 0.2f;
    ...
    private bool CanMoveFully(Vector2 direction)
    {
        ...
        if (hit.collider.CompareTag(doorTag)) return true; // pacman atraviesa puertas
    ...
    public void ReceiveDamage()
    {
        ...
        {
            isImmortal = true;
            Manager.Instance.DamagePlayer(); // aquí restará una vida
